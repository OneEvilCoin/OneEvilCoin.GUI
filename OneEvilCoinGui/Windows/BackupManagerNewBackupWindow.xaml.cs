    ...
    public partial class BackupManagerNewBackupWindow
    {
        ...
        private async void ButtonOk_Click(object sender, RoutedEventArgs e)
        {
            if (IsDefaultLocationSelected) {
                BackupDirectory = await StaticObjects.OneEvilCoinClient.Wallet.BackupAsync();
                DialogResult = true;
                ...
                if (dialog.ShowDialog() == true) {
                    BackupDirectory = await StaticObjects.OneEvilCoinClient.Wallet.BackupAsync(dialog.SelectedPath);
                    DialogResult = true;
                }
            }
